...
Sub RunMainframeScrape()
    ...
    ' --- BUILD THE VBSCRIPT (ROLLED BACK VERSION) ---
    vbsCode = "Option Explicit" & vbCrLf
    vbsCode = vbsCode & "Dim objSess, objPS, objOIA, objFSO, objFile, objLog, r, txt, buf, rows, cols, clist, i, sName, regEx, pageCount" & vbCrLf
    ...
    vbsCode = vbsCode & "pageCount = 0" & vbCrLf
    
    vbsCode = vbsCode & "Do" & vbCrLf
    vbsCode = vbsCode & "  objOIA.WaitForInputReady" & vbCrLf
    vbsCode = vbsCode & "  pageCount = pageCount + 1" & vbCrLf
    
    ' Scrape Rows 8 to 22 with Regex
    ...
    ' EXIT CONDITION (Added Circuit Breaker)
    vbsCode = vbsCode & "  buf = UCase(objPS.GetText(1, 1, rows * cols))" & vbCrLf
    vbsCode = vbsCode & "  If InStr(1, buf, ""LAST PAGE"") > 0 Or InStr(1, buf, ""INVALID"") > 0 Or pageCount > 50 Then Exit Do" & vbCrLf
    
    ' Advance Page (Simple Wait)
